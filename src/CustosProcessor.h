...
private:
    // AudioProcessorListener (on the inner synth) — UNION of #11 (rebind) + #12 (Learn):
    //   audioProcessorChanged: only parameterInfoChanged triggers a re-bind of the facade (#11).
    //   audioProcessorParameterChanged: per-value moves route into learnRecord (#12, RT-safe, gated on learnActive).
    // Single listener, single add/removeListener pair (in loadInner). Definitions in the .cpp.
    ...
    // (Learn's listener overrides are declared above, unioned with #11's rebind — see the
    // AudioProcessorListener block near the top of the private section.)
