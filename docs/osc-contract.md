...
| `/custos/window/rect` | `N, x,y,w,h:int, movable:int` | synth-window position feedback — emitted when the operator **drags** the window (on mouse-up) or when a rect is (re)applied. **Desktop-logical px (DIPs)**, same unit as the command (amended 2026-07-19). Lets KM capture the operator-chosen geometry for its settings |
